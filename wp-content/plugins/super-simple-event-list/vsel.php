...
function vsel_metabox_callback( $post ) {
  ...
  $event_video = get_post_meta( $post->ID, 'event-video', true );
  ...
  <p><label for="vsel-video"><?php _e( 'Link to video', 'very-simple-event-list' ); ?></label>
  <input class="widefat" id="vsel-video" type="text" name="vsel-video" maxlength="350" placeholder="<?php _e( 'Example: www.youtube.org', 'very-simple-event-list' ); ?>" value="<?php echo esc_url( $event_video ); ?>" /></p>
	<?php
}
...
function vsel_save_event_info( $post_id ) {
  ...
  if ( isset( $_POST['vsel-video'] ) ) {
		update_post_meta( $post_id, 'event-video', esc_url_raw( $_POST['vsel-video'] ) );
	}
