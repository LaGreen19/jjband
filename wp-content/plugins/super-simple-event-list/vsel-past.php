...
function vsel_past_events_shortcode( $vsel_atts ) {
	...
	$output .= '<div id="vsel">';
      ...
      $event_video = get_post_meta( get_the_ID(), 'event-video', true );
      $event_video_label = "Watch this Show";
          ...
          if (empty($event_link)){
						if ( ($event_link_hide == 'no') && ($widget_link == 'no') ) {
							$output .= '<p class="vsel-meta-link vsel-page-hide vsel-widget-hide">';
						} elseif ( ($event_link_hide != 'yes') && ($widget_link == 'yes') ) {
							$output .= '<p class="vsel-meta-link vsel-widget-hide">';
						} elseif ( ($event_link_hide == 'yes') && ($widget_link != 'yes') ) {
						...
						$output .= sprintf( esc_attr($event_link_label) );
						$output .= '</p>';
					}
          if (!empty($event_video)){
						if ($widget_video == 'yes') {
							$output .= '<p class="vsel-video">';
						} else {
							$output .= '<p class="vsel-meta-video">';
						}
						$output .= sprintf( '<a href="%1$s"'. $link_target .'>%2$s</a>', esc_url($event_video), esc_attr($event_video_label) );
						$output .= '</p>';
					}
				$output .= '</div>';
